...
//		$this->image["format"] = strtoupper($this->image["format"]);
                            $arrayFile = explode('.',$imgfile);
		$this->image["format"] = strtoupper(end($arrayFile));
	...
	public function save($save="")
	{
		...
		if ( empty($save) ) {
			$save = strtolower("./thumb.".$this->image["outputformat"]);
                                            ...
                                            $arrayFile = explode('.',$save);
                                           $this->image["outputformat"] = strtoupper(end($arrayFile));
...
//			$this->image["outputformat"] = strtoupper($this->image["outputformat"]);
        ...
        /**
         * Public class simple crop by Brieuc
         */
        public function createThumb(){
                        $targ_w =   config('global.picture_small_width');
                        $targ_h =   config('global.picture_small_height');
                        $jpeg_quality = 95;

                        $src = $serveur_path_url . $objPic->getPicPath();
                        $dst_file = $serveur_path_url . $objPic->getPicPath('thumb');
                        $img_r = imagecreatefromjpeg($src);
                        $dst_r = ImageCreateTrueColor( $targ_w, $targ_h );
                        
                        imagecopyresampled($dst_r,$img_r,0,0,
                        $startX,$startY,
                        $targ_w,$targ_h,$src_width,$src_height);

                        imagejpeg($dst_r,$dst_file,$jpeg_quality);
        }
